...
while (!$game_over) {
    for my $player (sort keys %players) {
        ...
        do {
            ...
            $choice = <STDIN>;
            chomp $choice;
        } until ($choice =~ /^\d+$/ && $choice >= 1 && $choice <= scalar(@hand));
        ...
        if ($played_card =~ / KM$/) {
            ...
            print "$player played $played_card. Total distance: $players{$player}{distance}\n";
...
my $winner = (sort { $players{$b}{distance} <=> $players{$a}{distance} } keys %players)[0];
